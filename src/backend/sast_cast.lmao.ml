...
let styp_of_sexpr = function
  ...
  | SLoopCtr -> SInt
  | SPeek2Anon t -> t
  | SExprDud -> bug "somebody is using duds"
          ...
          in let walk_primitive =
            let lit t l =
              let tr_lit = match l with
                ...
                | SLitStr s -> CLitStr s
                | _ -> CLitDud
                | _ -> bug "encountered collection type literal in walk_primitive"
            ...
            in let walk_binop t l o r =
              let tr_binop = match o with
                ...
                | _ -> CBinopDud
                | _ -> bug "encountered invalid binary operator in walk_primitive"
              ...
              in let dereference = (* operators whose operands are of Array t and int *)
                let eval =  (* TODO: make sure I understand what the fuck is going on here *)
                  ...
                  in let ind_t = if ind_t=SInt then ind_t else SInt 
                    (* bug "encountered type mismatch in dereference in walk_primitive" *)
              ...
              in match o with
                ...
                | SBinopFn _ -> acc
                | SBinopFn _ -> bug "encountered functional binop in walk_primitive"
                | SBinopGn _ -> acc
                | SBinopGn _ -> bug "encountered generator binop in walk_primitive"
                | _ -> primitive
            ...
            in let walk_cond t iff the els =
              ...
              in let cond_t = if cond_t=SBool then cond_t else SBool
                (* bug "non-boolean conditional expression in walk_primitive" *)
            ...
            in match rexpr with
              ...
              | _ -> acc
              | SGnCall(_, _, _) -> bug "encountered naked generator call in walk_primitive"
              | _ -> bug "encountered unexpected catch-all in walk_primitive"
          ...
          in let walk_array element_t element_c =
            ...
            in let lit t l =
              let l = match l with (* unwrap to list of expressions, emit by value *)
                | SLitArray l -> l
                | _ -> []
                | _ -> bug "encountered non-array type literal in walk_array"
              in let et = element_t
              in let at = if t=rtyp then t else t
                (* bug "literal type mismatch in walk_array" *)
              in let assign e i =
            ...
            in let walk_cond t iff the els = (* reference *)
              let cond_t = if styp_of_sexpr iff=SBool then SBool else SBool 
                (* bug "non-boolean conditional expression in walk_array" *)
              in let eval_merge = CExpr(t, CAssign(t, CPeek2Anon t, CPeekAnon t))
              in let eval_iff = push_anon_nop cond_t iff
              in let eval_the = push_anon t the eval_merge
              in let eval_els = push_anon t els eval_merge
            ...
            in let walk_binop t l o r =
              let tr_binop = match o with
                ...
                | _ -> CBinopDud
                | _ -> bug "tried to translate a binop that isn't SIndex in walk_array"
              ...
              in let generator =
                ...
                | _ -> acc
                | SGnCall(_, _, _) -> bug "gn call type mismatch in walk_array"
                |  _ -> bug "encountered non-SGnCall in right operand of SFor"
              ...
              in match o with
                ...
                | _ -> acc
                | _ -> bug "encountered invalid binop for walk_array"
            ...
            in match rexpr with
              ...
              | SLoopCtr -> bug "encountered SLoopCtr in walk_array"
              | SUnop(t, o, e) -> bug "encountered unop in walk_array"
              (* should never be called like this *)
              | SGnCall(_, _, _) -> bug "encountered naked SGnCall in walk_array"
              | _ -> bug "encountered unexpected catch-all expression in walk_array"
          ...
          in let walk_struct id members =
            ...
            in let lit t l =
              let (id, l) = match l with
                ...
                | _ -> ("", [])
                | _ -> bug "encountered non-struct type literal in walk_struct"
            ...
            in match rexpr with 
              ...
              | SPeek2Anon t -> emit t (CPeek2Anon t) :: acc
              | _ -> acc
              | _ -> bug "encountered unexpected catch-all expression in walk_struct"
          ...
          let rec lvalue_tr typ ass anon =
             let primitive_assign =
              let rec tr = function
                ...
                | _ -> CExprDud
                | SLoopCtr -> bug "should not be able to assign to LoopCtr in lvalue_tr"
                | _ -> bug "encountered non-lvalue in lvalue_tr"
            ...
            in match typ with
              ...
              | SArray(t, n) -> CStmtDud
              | SArray(_, None) -> bug "encountered None-size array type in lvalue_tr"
              | SArray(_, None) -> CStmtDud
              | SStruct(i, b) -> struct_assign i b
              | SPtr -> CStmtDud
              | SPtr -> bug "encountered pointer type in lvalue_tr"
              | SVoid -> if ass=[] then CBlock [] else CStmtDud
                (* bug "encountered assignment to void type in lvalue_tr" *)
              | _ -> primitive_assign

          in let rec walk ass = function
            | SAssign(t, l, r) when t=ltyp -> walk (l :: ass) r
            | SAssign(_, _, _) -> []
            | SAssign(_, _, _) -> bug "encountered assignment type mismatch in lvalue_tr"
  ...
  in let walk_kn kn =
    ...
    in let walk_ret ret = 
      ...
      in match ret with
        ...
        | None -> None
        | None -> bug "encountered None return expr in reference-returning kn in walk_kn"
    ...
    in let kn = match kn.skret_typ with
      ...
      | _ -> kn
      | SArray(t, None) -> bug "encountered kn that returns None sized array type in walk_kn"
      | SStruct(_, _) -> ref_kn
      | _ -> kn
  ...
  in let walk_gn gn = 
    ...
    in let st_fields =
      let a_decl = function
        ...
        | SBind(t, n, s) -> SBind(t, n, s)
        | SBind(_, n, _)-> bug "encountered non-SLocalVal binding in gn formals or local vals in walk_gn"
      in let ctr_decl =
    ...
    in let gn_to_kn =
      ...
      in let prefix_var = function
        ...
        | SBind(t, n, s) -> SBind(t, n, s)
        | SBind(_, n, _)-> bug "encountered non-SLocalVar in gn local vars in walk_gn"
      ...
      in let rec lookback (e, t) =
        let sid t id = function
          ...
          | SStructField as s -> SId(t, id, s)
          | SStructField -> bug "encountered SStructField binding scope in walk_gn"
